        ...
        const String8& rootpath, const String8& filepath, BackupDataWriter* writer)
{
    ...
    int err = 0;
    struct __stat64 s;//struct stat64 s;
    if (_stat64(filepath.string(), &s) != 0) {//if (lstat64(filepath.string(), &s) != 0) {
        err = errno;
...
backup_helper_test_empty()
{
    ...
    _mkdir(SCRATCH_DIR);//mkdir(SCRATCH_DIR, 0777);
...
backup_helper_test_data_writer()
{
    ...
    system("rm -r " SCRATCH_DIR);
    _mkdir(SCRATCH_DIR);//mkdir(SCRATCH_DIR, 0777);
    _mkdir(SCRATCH_DIR "data");//mkdir(SCRATCH_DIR "data", 0777);
...
{
    int err;
    struct __stat64 st;//struct stat64 st;
    err = _stat64(filename, &st);//err = stat64(filename, &st);
    if (err != 0) {
...
#else
    times[0].tv_usec = 0;//times[0].tv_usec = st.st_atime_nsec / 1000;
    times[1].tv_usec = 0;//times[1].tv_usec = st.st_mtime_nsec / 1000;
#endif
